...
async function bookWith(app: App, cookie: string, slug: string, time: string) {
...
describe("payments — no-custody, gated, idempotent (I12, I14, I15, I20)", () => {
  it("an unverified provider cannot take an in-app payment by any path", async () => {
    ...
    const booking = await bookWith(app, c.cookie, "hydro-hand-wash", "09:00");
  ...
  it("a verified provider takes payment with zero platform fee and no custody", async () => {
    ...
    const booking = await bookWith(app, c.cookie, "jamies-mobile-valet", "10:00");
    const pay = await request(app)
      .post(`/api/bookings/${booking.id}/pay`)
      .set("Cookie", c.cookie)
      .send({ idempotencyKey: "key-1" });
    expect(pay.status).toBe(200);
    expect(pay.body.applicationFee).toBe(0);
  ...
  it("is idempotent — the same key never double-charges", async () => {
    ...
    const count = await db.get<{ n: number }>(`SELECT COUNT(*) AS n FROM payments WHERE booking_id = ?`, [booking.id]);
  ...
  it("capture settles to the provider's connected account (platform balance stays zero)", async () => {
    ...
    const row = await db.get<{ destination_account: string; application_fee: number }>(
      `SELECT destination_account, application_fee FROM payments WHERE id = ?`,
      [pay.body.paymentId],
    );
    expect(row!.destination_account).toContain("acct_");
    expect(Number(row!.application_fee)).toBe(0);
  });
});
