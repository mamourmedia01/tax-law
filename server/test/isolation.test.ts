...
import { freshApp, registerCustomer, type App } from "./helpers.js";

async function book(app: App, cookie: string, slug = "jamies-mobile-valet", time = "10:00") {
...
describe("tenant isolation (I7, I35, I36)", () => {
  ...
  it("a provider cannot pay another customer's booking (deny-by-default)", async () => {
    ...
    const res = await request(app).post(`/api/bookings/${booking.body.id}/pay`).set("Cookie", b.cookie).send({});
    expect(res.status).toBe(403);
  });
});
